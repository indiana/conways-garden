...
export class AchievementsScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      .text(LAYOUT.CENTER_X, LAYOUT.ACHIEVEMENTS_TITLE_Y, "", {
    ...
    this.gameStateManager.subscribeAchievements(() =>
  ...
  private renderAchievements() {
    ...
    achievementsList.forEach((ach) => {
      ...
      const itemContainer = this.add.container(90, currentY);

      const icon = this.add.sprite(0, 0, iconKey).setScale(1);
      const title = this.add
        .text(40, 0, this.localeManager.get(ach.titleKey).toUpperCase(), {
          fontFamily: "Comfortaa, sans-serif",
          fontSize: "32px",
          fontStyle: "bold",
          color: textColor,
      ...
      if (isUnlocked) {
        const check = this.add
          .text(title.x + title.width + 10, 0, "✔️", { fontSize: "32px" })
          .setOrigin(0, 0.5);
        line1Elements.push(check);
      }

      const desc = this.add
        .text(40, 40, this.localeManager.get(ach.descriptionKey), {
          fontFamily: "Comfortaa, sans-serif",
          fontSize: "24px",
          fontStyle: "italic",
          color: textColor,
      ...
      const bonusValueWidth = 560 - bonusLabelWidth;

      const bonusLabel = this.add
        .text(
          40,
          desc.y + desc.displayHeight + 10,
          this.localeManager.get("ACHIEVEMENT_UNLOCKS"),
          {
            fontFamily: "Comfortaa, sans-serif",
            fontSize: "24px",
            fontStyle: "bold",
      ...
      const bonusValue = this.add
        .text(
          bonusLabel.x + bonusLabel.width,
          bonusLabel.y,
          this.localeManager.get(ach.rewardDescriptionKey),
          {
            fontFamily: "Comfortaa, sans-serif",
            fontSize: "24px",
            color: isUnlocked ? "#f1c40f" : "#7f8c8d",
      ...
      const itemHeight =
        Math.max(
          bonusLabel.y + bonusValue.displayHeight,
          desc.y + desc.displayHeight,
        ) + 50;
      currentY += itemHeight;
    });
